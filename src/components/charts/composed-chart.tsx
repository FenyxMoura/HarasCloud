...
/** Mede o container sem depender de ResizeObserver (que pode não disparar
 *  em alguns ambientes), com fallback em resize/intervalo. */
function useChartSize<T extends HTMLElement>() {
  ...
  useLayoutEffect(() => {
    ...
    medir();
    // Re-mede depois que o layout estabiliza (fontes/imagens/página-in)
    tempo = window.setTimeout(medir, 120);
    ...
    return () => {
      if (tempo !== undefined) window.clearTimeout(tempo);
...
export function ComposedChart({
  ...
  const { ref: containerRef, width, height } = useChartSize<HTMLDivElement>();
  const margin = { ...DEFAULT_MARGIN, ...marginProp };
  const pronto = width > 0 && height > 0;
